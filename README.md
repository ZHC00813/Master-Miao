...
本仓库保存可复现构建的源代码与验证文档，不包含用户项目、运行时 `Data`、SolidWorks 测试零件或本机缓存。可直接运行的 V1.2.5 压缩包发布在仓库的 Releases 页面。
...
3. 使用列表模式批量浏览、Ctrl/Shift 多选和批量分类；双击“导出名称”后可连续输入和使用输入法选字，点击另一单元格或“命名完毕”才提交，Esc 取消；也可打开“逐项整理”，一次专注一个零件并查看三张大图、重命名、分类或跳过。
...
- 针对中文输入法再次重构列表改名：不再使用 DataGridView 的短生命周期编辑控件，而是在名称单元格上放置持续存在的覆盖编辑框。
- 输入法选字或按 Enter 不会意外结束编辑；点击另一单元格、切换源文件或点击“命名完毕 / Finish naming”时才写回项目，Esc 可取消。
- 新增逐字符输入、模拟输入法 Enter、点击其他单元格提交以及改名界面截图测试；程序程序集和窗口标题升级为 `1.2.5.0` / `V1.2.5`。

![V1.2.5 输入法安全改名](docs/UI_NAME_EDIT.png)

## V1.2.4 功能

- 首次修复列表“导出名称”逐字符编辑被提前提交的问题，加入显式完成按钮和多字符自检；V1.2.5 在此基础上进一步解决中文输入法选字上屏场景。
...
V1.1.2 的同一导出内核已在真实用户桌面完成三实体全自动验收：3 个 SLDPRT、3 个零件 STEP、1 个原位装配体、1 个装配体 STEP 和 1 份 Excel 报表全部成功。V1.2.4 增加验证后的双目录路由；V1.2.5 只修改列表改名交互，没有修改 SolidWorks 宏或装配体批量导出顺序。回归结果和桌面权限限制详见 `VALIDATION.md`。
